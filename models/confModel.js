...
class Conf {
  constructor(confFilePath) {
    console.log(confFilePath); // Log the file path for debugging
    this.conf = confFilePath ? new nedb(confFilePath) : new nedb(); // Initialize NeDB with or without a file path
  }
  ...
  init() {
    const initialTalks = [
      ...
      },
      {
        ...
        description:
          "Understand the technical foundations, as well as the non-programming skills needed to be a successful full stack web developer. This session reveals the reasons why a truly successful full stack developer does more than write code.",
        session: "A",
        time: "12:00",
      ...
      },
      {
        ...
        description:
          "If you want to learn how to build efficient React applications, this is your talk. Ideal for web developers and software engineers who understand how JavaScript, CSS, and HTML work in the browser, this session provides best practices and patterns for writing modern React code.",
        session: "A",
        time: "14:00",
      ...
      },
      {
        ...
        description:
          "Assemble the complete stack required to build a modern web app using MongoDB, Express, React, and Node. This session also covers many other complementary tools: React Router, GraphQL, React-Bootstrap, Babel, and Webpack.",
        session: "B",
        time: "10:30",
      ...
      },
      {
        ...
        description:
          "This session shows you how to use the emerging Web Real-Time Communication (WebRTC) technology to build a browser-to-browser application, piece by piece.",
        session: "B",
        time: "14:00",
      ...
      },
      {
        ...
        description:
          "Discover the simplicity of Redis to make web applications faster than before. Create unique web servers and networking tools with Node.js. Implement real-time applications on websites using Socket.io with Express and Redis.",
        session: "B",
        time: "15:30",
      ...
      },
      {
        ...
        description:
          "Discover the semantics of HTML5 and Microdata Understand the concept of the CSS3 Flexible Box model Explore the main features of HTML5 such as canvas, offline web application, geolocation, audio and video elements, and web storage Master the tools and utilities in HTML5 and CSS3 In Detail.",
        session: "C",
        time: "10:30",
        tags: ["CSS", "HTML5"],
        ratings: [],
      ...
      },
      {
        ...
        description:
          "Introduces Cross-Origin Resource Sharing (CORS) from both the server and the client perspective. It starts with the basics: how to make CORS requests and how to implement CORS on the server. It then explores key details such as performance, debugging, and security. API authors will learn how CORS opens their APIs to a wider range of users. JavaScript developers will find valuable techniques for building rich web apps that can take advantage of APIs hosted anywhere.",
        session: "C",
        time: "15:30",
        tags: ["CORS", "security"],
        ratings: [],
  ...
  // Fetch talks by speaker name
  getSpeaker(speakerName) {
    return new Promise((resolve, reject) => {
      this.conf.find({ speaker: speakerName }, (err, entries) => {
        if (err) reject(err); // Reject if there's an error
        else resolve(entries); // Resolve with entries matching the speaker
      });
    });
  }

  // Fetch talks by session name
  getSession(sessionName) {
    return new Promise((resolve, reject) => {
      this.conf.find({ session: sessionName }, (err, entries) => {
        if (err) reject(err); // Reject if there's an error
        else resolve(entries); // Resolve with entries matching the session
      });
    });
  }
  ...
  getTime(talkTime) {
    return new Promise((resolve, reject) => {
      this.conf.find({ time: talkTime }, (err, entries) => {
        if (err) reject(err); // Reject if there's an error
        else resolve(entries); // Resolve with entries matching the time
      });
    });
  }

  // Fetch a talk by its ID
  getTalkById(id) {
    return new Promise((resolve, reject) => {
      this.conf.find({ id }, (err, entries) => {
        if (err) reject(err); // Reject if there's an error
        else resolve(entries); // Resolve with entries matching the ID
      });
    });
  }

  // Add a rating to a talk by its ID
  rateTalkById(id, newRating) {
    const rating = Number(newRating); // Convert rating to a number
    return new Promise((resolve, reject) => {
      this.conf.update(
        { id },
        { $push: { ratings: rating } },
        ...
        (err, numUpdated) => {
          if (err) reject(err); // Reject if there's an error
          else resolve(numUpdated); // Resolve with the number of updated entries
        }
      );
    });
  }
